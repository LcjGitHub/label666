...
can_edit_config = has_permission("edit_config")
...
def init_subscription_state(username):
    state_key = f"sub_state_{username}"
    if state_key not in st.session_state:
        user_sub = get_user_subscription(username)
        st.session_state[state_key] = {
            "enabled": user_sub.get("enabled", True),
            "subscribed_rules": list(user_sub.get("subscribed_rules", list(DEFAULT_ALERT_RULES.keys()))),
            "rule_channels": {k: list(v) for k, v in user_sub.get("rule_channels", DEFAULT_USER_SUBSCRIPTION["rule_channels"]).items()}
        }
    return st.session_state[state_key]


def reset_subscription_state(username):
    state_key = f"sub_state_{username}"
...
with tab_rules:
    ...
    st.subheader("📝 订阅规则配置")
    st.markdown("配置个性化的通知订阅规则，选择需要接收的通知类型和接收渠道。")
    st.markdown("---")
    ...
    if can_edit_config:
        ...
        with col_user_select:
            target_username = st.selectbox(
                ...
                index=all_usernames.index(current_username) if current_username in all_usernames else 0,
                format_func=lambda x: next((f"{u.get('full_name', x)} ({x})" for u in users if u["username"] == x), x),
                key="sub_target_user"
            )
            if st.session_state.get("_prev_target_user") != target_username:
                st.session_state["_prev_target_user"] = target_username
        ...
        target_user_info = next((u for u in users if u["username"] == target_username), None)
        if target_user_info:
            st.info(f"当前用户: **{target_user_info.get('full_name', target_username)}** ({target_username})  |  邮箱: {target_user_info.get('email', '未设置')}")

    sub_state = init_subscription_state(target_username)
    ...
    with col_enabled:
        sub_enabled = st.checkbox(
            "启用通知订阅",
            value=sub_state["enabled"],
            help="关闭后将不会收到任何通知",
            key=f"sub_enabled_{target_username}"
        )
        sub_state["enabled"] = sub_enabled

    with col_quick:
        st.markdown("**快捷操作**")
        col_q1, col_q2, col_q3 = st.columns(3)
        with col_q1:
            if st.button("📋 全选所有规则", use_container_width=True, key=f"select_all_{target_username}"):
                sub_state["subscribed_rules"] = list(DEFAULT_ALERT_RULES.keys())
                for rule_id in DEFAULT_ALERT_RULES.keys():
                    if rule_id not in sub_state["rule_channels"]:
                        sub_state["rule_channels"][rule_id] = list(DEFAULT_ALERT_RULES[rule_id].get("notify_channels", ["in_app"]))
                st.rerun()
        with col_q2:
            if st.button("❌ 清空所有订阅", use_container_width=True, key=f"clear_all_{target_username}"):
                sub_state["subscribed_rules"] = []
                st.rerun()
        with col_q3:
            if st.button("🔄 恢复默认设置", use_container_width=True, key=f"reset_default_{target_username}"):
                reset_subscription_state(target_username)
                st.rerun()
    ...
    subscribed_rules = sub_state["subscribed_rules"]
    rule_channels = sub_state["rule_channels"]
    ...
    for rule_id, default_rule in DEFAULT_ALERT_RULES.items():
        ...
        with st.container():
            ...
            with col_sub:
                ...
                if sub_this:
                    if rule_id not in sub_state["subscribed_rules"]:
                        sub_state["subscribed_rules"].append(rule_id)
                else:
                    if rule_id in sub_state["subscribed_rules"]:
                        sub_state["subscribed_rules"].remove(rule_id)

            with col_ch:
                if sub_this and sub_enabled:
                    st.markdown("**接收渠道**:")
                    if rule_id not in sub_state["rule_channels"]:
                        sub_state["rule_channels"][rule_id] = list(default_rule.get("notify_channels", ["in_app"]))
                    current_channels = sub_state["rule_channels"][rule_id]
                    selected_chs = []
                    ch_cols = st.columns(len(NOTIFICATION_CHANNELS))
                    for idx, (ch_id, ch_info) in enumerate(NOTIFICATION_CHANNELS.items()):
                        with ch_cols[idx]:
                            ch_checked = st.checkbox(
                                f"{ch_info['name']}",
                    ...
                    if selected_chs:
                        sub_state["rule_channels"][rule_id] = selected_chs
    ...
    with col_sum1:
        st.metric("已订阅规则数", len(sub_state["subscribed_rules"]), f"共 {len(DEFAULT_ALERT_RULES)} 个规则")
    with col_sum2:
        enabled_count = sum(1 for rid in sub_state["subscribed_rules"] if rules.get(rid, {}).get("enabled", True))
        st.metric("有效订阅（规则已启用）", enabled_count)
    with col_sum3:
        has_email = any("email" in sub_state["rule_channels"].get(rid, []) for rid in sub_state["subscribed_rules"])
    ...
    with col_save:
        if st.button("💾 保存订阅设置", use_container_width=True, type="primary", key=f"save_sub_{target_username}"):
            new_subscription = {
                "enabled": sub_state["enabled"],
                "subscribed_rules": list(sub_state["subscribed_rules"]),
                "rule_channels": {k: list(v) for k, v in sub_state["rule_channels"].items()}
            }
    ...
    with col_test:
        if st.button("🧪 发送测试通知", use_container_width=True, key=f"test_sub_{target_username}"):
            test_rules_to_send = sub_state["subscribed_rules"] if sub_state["enabled"] else []
            if not test_rules_to_send:
                ...
                test_channels = sub_state["rule_channels"].get(test_rule_id, ["in_app"])
                test_notif = create_notification(
...
with tab_channels:
    ...
    with col_ch2:
        ...
        if can_edit_config:
            ...
            if email_enabled:
                st.success("✅ 邮件通知渠道已启用")
                st.info(f"发件人: {current_email.get('sender_name', '')} <{current_email.get('smtp_user', '')}>")
            else:
                st.info("邮件通知渠道未启用。")
            st.info("ℹ️ 邮件配置需管理员权限修改。")
